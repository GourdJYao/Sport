package com.yaojian.model;

public class AdvertisementInfo {
	private Integer advertiseid;
	private Integer serverid;
	private String yaopingmingzi;
	private String yaopingguanggaopizhuwenhao;
	private String danweimingchen;
	private String dizhi;
	private String youzhengbianma;
	private String tongyongmingchen;
	private String shangpingmingcheng;
	private String shangbiaomingcheng;
	private String chufangneixing;
	private String guanggaoleixing;
	private String shichang;
	private String guanggaoyouxiaoqi;
	private String guanggaoneirong;
	private String pizhunweihao;
	private String beizhu;
	
	public Integer getAdvertiseid() {
		return advertiseid;
	}
	public void setAdvertiseid(Integer advertiseid) {
		this.advertiseid = advertiseid;
	}
	public String getYaopingmingzi() {
		return yaopingmingzi;
	}
	public void setYaopingmingzi(String yaopingmingzi) {
		this.yaopingmingzi = yaopingmingzi;
	}
	public String getYaopingguanggaopizhuwenhao() {
		return yaopingguanggaopizhuwenhao;
	}
	public void setYaopingguanggaopizhuwenhao(String yaopingguanggaopizhuwenhao) {
		this.yaopingguanggaopizhuwenhao = yaopingguanggaopizhuwenhao;
	}
	public String getDanweimingchen() {
		return danweimingchen;
	}
	public void setDanweimingchen(String danweimingchen) {
		this.danweimingchen = danweimingchen;
	}
	public String getDizhi() {
		return dizhi;
	}
	public void setDizhi(String dizhi) {
		this.dizhi = dizhi;
	}
	public String getYouzhengbianma() {
		return youzhengbianma;
	}
	public void setYouzhengbianma(String youzhengbianma) {
		this.youzhengbianma = youzhengbianma;
	}
	public String getTongyongmingchen() {
		return tongyongmingchen;
	}
	public void setTongyongmingchen(String tongyongmingchen) {
		this.tongyongmingchen = tongyongmingchen;
	}
	public String getShangpingmingcheng() {
		return shangpingmingcheng;
	}
	public void setShangpingmingcheng(String shangpingmingcheng) {
		this.shangpingmingcheng = shangpingmingcheng;
	}
	public String getShangbiaomingcheng() {
		return shangbiaomingcheng;
	}
	public void setShangbiaomingcheng(String shangbiaomingcheng) {
		this.shangbiaomingcheng = shangbiaomingcheng;
	}
	public String getChufangneixing() {
		return chufangneixing;
	}
	public void setChufangneixing(String chufangneixing) {
		this.chufangneixing = chufangneixing;
	}
	public String getGuanggaoleixing() {
		return guanggaoleixing;
	}
	public void setGuanggaoleixing(String guanggaoleixing) {
		this.guanggaoleixing = guanggaoleixing;
	}
	public String getShichang() {
		return shichang;
	}
	public void setShichang(String shichang) {
		this.shichang = shichang;
	}
	public String getGuanggaoyouxiaoqi() {
		return guanggaoyouxiaoqi;
	}
	public void setGuanggaoyouxiaoqi(String guanggaoyouxiaoqi) {
		this.guanggaoyouxiaoqi = guanggaoyouxiaoqi;
	}
	public String getGuanggaoneirong() {
		return guanggaoneirong;
	}
	public void setGuanggaoneirong(String guanggaoneirong) {
		this.guanggaoneirong = guanggaoneirong;
	}
	public String getPizhunweihao() {
		return pizhunweihao;
	}
	public void setPizhunweihao(String pizhunweihao) {
		this.pizhunweihao = pizhunweihao;
	}
	public String getBeizhu() {
		return beizhu;
	}
	public void setBeizhu(String beizhu) {
		this.beizhu = beizhu;
	}
	public Integer getServerid() {
		return serverid;
	}
	public void setServerid(Integer serverid) {
		this.serverid = serverid;
	}
	
}

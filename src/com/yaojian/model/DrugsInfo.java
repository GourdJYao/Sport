package com.yaojian.model;

public class DrugsInfo {
	private Integer id;
	private Integer cfdaid;
	private String drugsname;
	private String pizhuwenhao;
	private String yingwenmingchen;
	private String shangpingming;
	private String jixing;
	private String guige;
	private Integer shengchandanweiid;
	private String shengchangdanweiname;
	private String shengchandizhi;
	private String chanpinleixing;
	private String pizhunriqi;
	private String yuanpizhuwenhao;
	private String yaopinbenweima;
	private String yaopinbenweimabeizhu;
	private String gmplist;

	public Integer getId() {
		return id;
	}

	public void setId(Integer id) {
		this.id = id;
	}

	public Integer getCfdaid() {
		return cfdaid;
	}

	public void setCfdaid(Integer cfdaid) {
		this.cfdaid = cfdaid;
	}

	public String getDrugsname() {
		return drugsname;
	}

	public void setDrugsname(String drugsname) {
		this.drugsname = drugsname;
	}

	public String getPizhuwenhao() {
		return pizhuwenhao;
	}

	public void setPizhuwenhao(String pizhuwenhao) {
		this.pizhuwenhao = pizhuwenhao;
	}

	public String getYingwenmingchen() {
		return yingwenmingchen;
	}

	public void setYingwenmingchen(String yingwenmingchen) {
		this.yingwenmingchen = yingwenmingchen;
	}

	public String getShangpingming() {
		return shangpingming;
	}

	public void setShangpingming(String shangpingming) {
		this.shangpingming = shangpingming;
	}

	public String getJixing() {
		return jixing;
	}

	public void setJixing(String jixing) {
		this.jixing = jixing;
	}

	public String getGuige() {
		return guige;
	}

	public void setGuige(String guige) {
		this.guige = guige;
	}

	public Integer getShengchandanweiid() {
		return shengchandanweiid;
	}

	public void setShengchandanweiid(Integer shengchandanweiid) {
		this.shengchandanweiid = shengchandanweiid;
	}

	public String getShengchangdanweiname() {
		return shengchangdanweiname;
	}

	public void setShengchangdanweiname(String shengchangdanweiname) {
		this.shengchangdanweiname = shengchangdanweiname;
	}

	public String getShengchandizhi() {
		return shengchandizhi;
	}

	public void setShengchandizhi(String shengchandizhi) {
		this.shengchandizhi = shengchandizhi;
	}

	public String getChanpinleixing() {
		return chanpinleixing;
	}

	public void setChanpinleixing(String chanpinleixing) {
		this.chanpinleixing = chanpinleixing;
	}

	public String getPizhunriqi() {
		return pizhunriqi;
	}

	public void setPizhunriqi(String pizhunriqi) {
		this.pizhunriqi = pizhunriqi;
	}

	public String getYuanpizhuwenhao() {
		return yuanpizhuwenhao;
	}

	public void setYuanpizhuwenhao(String yuanpizhuwenhao) {
		this.yuanpizhuwenhao = yuanpizhuwenhao;
	}

	public String getYaopinbenweima() {
		return yaopinbenweima;
	}

	public void setYaopinbenweima(String yaopinbenweima) {
		this.yaopinbenweima = yaopinbenweima;
	}

	public String getYaopinbenweimabeizhu() {
		return yaopinbenweimabeizhu;
	}

	public void setYaopinbenweimabeizhu(String yaopinbenweimabeizhu) {
		this.yaopinbenweimabeizhu = yaopinbenweimabeizhu;
	}

	public String getGmplist() {
		return gmplist;
	}

	public void setGmplist(String gmplist) {
		this.gmplist = gmplist;
	}

}
